...
def join_lists(lst):
    ...
    ----------------------------
    joins every 4 strings in the input list into 1 
    """
    
    r=[''.join(x) for x in zip(lst[0::2], lst[1::2])]
    return [''.join(x) for x in zip(r[0::2], r[1::2])]
    return r
...
def decode_dump(dir):
    ...
    for file in files:
        ...
        if file.endswith('.raw'):
            command=["xxd","-c8",dir+'/'+file,dir+"/"+file[:-3]+"txt"]
...
def clean_offset_len(offset,length):
    ...
    for j in range(len(length)):       
        key_lengths[j] = int(ceil(float(length[j])/8))
...
def create_file_df(file_data):
    ...
    df = df.drop(df[(df['class'] == 1) & (df['hex_values'] == '0000 0000 0000 0000')].index)
    df = df.drop(df[(df['content'] == "........") & (df['hex_values'] == '0000 0000 0000 0000')].index)
    df = df.reset_index()
    return df

# Splits a file dataframe into a N_size sequence dataframe
def create_seq_df(df,N):
    """
    df : dataframe created by create_df_file function
    N : sequence size
    --------------------------------------------------------------------------------------------------------------------
    returns a dataframe having columns ['address_range','hex_values','values','classes','Label']
    ...
    for index, row in df.iterrows():  
        if (len(hex_list)< len(df)//N):
            if (counter< N  ):                
                ...
                temp_labels_list.append(df.iloc[index,4])
                counter=counter+1
        else: # last sequence will contain remaining values 
            temp_offset_list.append(df.iloc[index,1])
            temp_hex_list.extend(split_strings(df.iloc[index,2]))
            temp_content_list.append(df.iloc[index,3])
            temp_labels_list.append(df.iloc[index,4])
            counter=counter+1
    ###adding 0 padding to the last sequence until size ==N
    while (len(temp_hex_list) < 4*N ):
        temp_hex_list.extend(["0000","0000","0000","0000"])
    ...
    file_df['Label']= file_df['classes'].apply(lambda a: max(a))
    return file_df   




def file_save_set(data,file,N):
    """
    data : dataframe to save as csv
    file : folder path
    """
    post='segment_size_'+str(N)
    filename=file.split('/')
    filename_list1=filename[-3:]
    name='-'.join(filename_list1) 
    name_dir='./CSV_Data/'+post+'/'
    sub_dir = filename[2]
    name_file=''+name+'.csv'
...
#Takes in a Path and Sequence size
#creates sequence dataframes as CSV from all leaf directores in the path 
def transform(path,N):
    ...
    for direc in leaf_direc:
        ...
        text_files = sorted(list_files(direc, '.txt'))   ### a list containing all .txt file names in the leaf directory direc
        json_files = sorted(list_files(direc, '.json'))  ### a list containing all .json file names in the leaf directory direc
        ...
        for i in range(it):
            try:
                ...
                print('issue with: ',text_files[i])
                print(error)
                raise 
        folder_df=pd.concat(file_dfs)         ### joining all seq_df into one folder dataframe
        folder_df = folder_df.reset_index()   
        file_save_set(folder_df,str(direc),N)   ### saving the folder dataframe as csv file
    return

def decoding(path):
    folders= leaf_dirs(path)
